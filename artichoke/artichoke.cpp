...
signed main(void){
    ...
    double a,b,c,d,p,n; cin >> p >> a >> b >> c >> d >> n; 
    double mx_ans = 0.0f , mn_ans = 100000.0f;
    double ans{};
    for(auto i:range(1,n+1)){
        double cal = double(p * (sin(a * i + b) + cos(c * i + d) + 2.0f));
        if(i==1){
            mx_ans = cal;
            mn_ans = cal;
            continue;
        }
        mn_ans = min(mn_ans , cal);
        ans = max(ans ,fabs(mx_ans - mn_ans));
        if(mx_ans < cal){
            mx_ans = cal; 
            mn_ans = mx_ans;
        }
