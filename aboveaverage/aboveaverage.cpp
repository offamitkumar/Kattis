...
signed main(void){
    ...
    while(c--){
        ...
        double x = accumulate(v.begin() , v.end() , 0.0f);
        double avg = ((double)x)/(double)v.size();
        show(avg);
        double y =  count_if(v.begin() , v.end() ,
                [avg](double m){
                return double(m) >avg;
                }); 
        show(y);
        cout <<fixed << setprecision(3)<< ((double)y)/(double)v.size() * 100.0f << "%"<<'\n';
    }
