...
int max_sum(vector<int>::iterator b_itr , vector<int>::iterator e_itr){
    Int max_seen = -100000000;
    Int max_ans{};
    while(b_itr != e_itr){
        max_seen = max( (Int)*b_itr , max_seen + Int(*b_itr) );
        max_ans = max(max_ans , max_seen); 
        ++b_itr;
    }
    return max_ans; 
}
